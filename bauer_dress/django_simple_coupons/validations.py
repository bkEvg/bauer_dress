...
def validate_max_uses_rule(coupon_object, user):
    ...
    try:
        # coupon_user = CouponUser.objects.get(user=user) #this is from docs with issue
...
def validate_coupon(coupon_code, user):
    if not coupon_code:
        return assemble_invalid_message(message="Введите купон!")

    if not user:
        return assemble_invalid_message(message="Пользователь отсутствует!")

    try:
        coupon_object = Coupon.objects.get(code=coupon_code)
    except Coupon.DoesNotExist:
        return assemble_invalid_message(message="Такого купона не существует!")
    ...
    if not valid_allowed_users_rule:
        return assemble_invalid_message(message="Вам не разрешено использовать этот купон!")

    valid_max_uses_rule = validate_max_uses_rule(coupon_object=coupon_object, user=user)
    if not valid_max_uses_rule:
        return assemble_invalid_message(message="Вы достигли лимита использования этого купона!")

    valid_validity_rule = validate_validity_rule(coupon_object=coupon_object)
    if not valid_validity_rule:
        return assemble_invalid_message(message="Неправильный купон!")
